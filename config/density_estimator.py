...
def get_diffusion_config(dataset, standalone):
    if standalone:
        image_channels = 3
        x_shape = (1, 28, 28) if dataset in ["mnist", "fashion-mnist"] else (3, 32, 32)
        hidden_channels = 64
        ch_mults = (1, 2, 2, 4)
        is_attn = (False, False, True, True)
        test_metrics = ["loss", "fd"]
        metric_kwargs = {
                "fd": {"use_dinov2": True}  # False uses Inception instead of DINOv2 for FD
            }
    else:
        image_channels = 4  # this must be the number of channels in the latent representation of step 1
        x_shape = (4, 8, 8)  # this must match the shape of the latent representations from step 1
        hidden_channels = 256
        ch_mults = (1, 2)
        is_attn = (True, True)
        test_metrics = ["loss"]
