...
from . import TwoStepDensityEstimator, GaussianVAE, AdversariallyRegularizedVAE, AdversarialVariationalBayes
from .networks import MLP, CNN, T_CNN, GaussianMixtureLSTM, UNet, ResidualNetwork
from .networks import SimpleFlowTransform
from .generalized_autoencoder import AutoEncoder, WassersteinAutoEncoder, BiGAN
from .density_estimator import NormalizingFlow, EnergyBasedModel, GaussianMixtureLSTMModel, ScoreBasedDiffusionModel
...
def get_vae_module(cfg):
    ...
    return GaussianVAE(
        ...
        fixed_sigma=cfg["fixed_sigma"],
        **get_data_transform_kwargs(cfg)
    )


def get_adv_vae_module(cfg):
    encoder, decoder = get_encoder_decoder(cfg)
    discriminator = get_discriminator(cfg)

    return AdversariallyRegularizedVAE(
        latent_dim=cfg["latent_dim"],
        encoder=encoder,
        decoder=decoder,
...
def get_diffusion_module(cfg):
    score_network = UNet(
        image_channels=cfg["image_channels"],
        n_channels=cfg["hidden_channels"],
        ch_mults=cfg["ch_mults"],
        is_attn=cfg["is_attn"],
        n_blocks=cfg["n_blocks"],
    )

    return ScoreBasedDiffusionModel(
        score_network=score_network,
        x_shape=cfg["x_shape"],
        T=cfg.get("T", 1.),
        beta_min=cfg.get("beta_min", 0.1),
        beta_max=cfg.get("beta_max", 20.),
        **get_data_transform_kwargs(cfg)
    )
...
def get_encoder_decoder(cfg):
    ...
    elif cfg["encoder_net"] == "resnet":
        encoder = ResidualNetwork(
            type_="encoder",
            input_channels=cfg["data_shape"][0],
            output_channels=2*cfg["output_channels"],
            output_split_sizes=[cfg["output_channels"], cfg["output_channels"]],
    ...
    elif cfg["decoder_net"] == "resnet":
        decoder = ResidualNetwork(
            type_="decoder",
            input_channels=cfg["output_channels"],
            output_channels=cfg["data_shape"][0],
            output_split_sizes=None,
            n_channels=cfg["n_channels"] * prod(cfg["ch_mults"]),
            ch_mults=cfg["ch_mults"],
            n_blocks=cfg["n_blocks"],
        )
...
def get_discriminator(cfg):
    ...
    if cfg.get("discriminator_net", None) == "resnet":
        discriminator = ResidualNetwork(
            type_="encoder",
            input_channels=cfg["data_shape"][0],
...
def get_data_transform_kwargs(cfg):
    return {
        ...
        "scale_data_to_n1_1": cfg.get("scale_data_to_n1_1", False),
